...
public class Main extends PanelScreen
{
    ...
    private var _bounceBackMode:PickerList;
    ...
    private function customFooterFactory():ScrollContainer
    {
        ...
        if(!this._bounceBackMode)
        {
            this._bounceBackMode = new PickerList();
            this._bounceBackMode.dataProvider = new ListCollection([{mode:BounceBackMode.JUMP_TO_EDGE, label:"Jump To Edge"},{mode:BounceBackMode.STAY_IN_PLACE, label:"Stay In Place"}]);
            this._bounceBackMode.prompt = "Bounce Back";
            this._bounceBackMode.selectedIndex = -1;
            this._bounceBackMode.addEventListener(Event.CHANGE, bounceBackMode_changeHandler);
        }
        this._toolbar.addChild(this._bounceBackMode);
    ...
    private function bounceBackMode_changeHandler(event:Event):void
    {
        if (this._bounceBackMode.selectedItem)
            this.list.bounceBackMode = this._bounceBackMode.selectedItem.mode;
    }
