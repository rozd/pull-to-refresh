...
public class GroupedPullToRefreshBase extends GroupedList
{
    //--------------------------------------------------------------------------
    //
    //  Class constants
    ...
    public function GroupedPullToRefreshBase()
    {
        super();
    }
    ...
    {
        if (GroupedPullToRefreshBase.globalStyleProvider)
        {
            return GroupedPullToRefreshBase.globalStyleProvider;
        }
...
//                    footerAsDisplayObject.height = Math.max(actualHeight - footerAsDisplayObject.y, 0);
    ...
    protected function loadComplete(data:Array, hasOldRecords:Boolean = false, hasNewRecords:Boolean = true):void
    {
        ...
        if (_dataProvider == null)
            _dataProvider = new ArrayHierarchicalCollection();

        if (_insertDataFunction != null)
        {
            _insertDataFunction(data);
        }
        else if (_dataProvider is ArrayHierarchicalCollection)
        {
            ArrayHierarchicalCollection(_dataProvider).arrayData = data;
        }

        if (_dataProvider.getLength() == 0)
        {
    ...
    protected function refreshResult(items:Array, hasNewRecords:Boolean = true):void
    {
        ...
        else
        {
            ...
            {
                _dataProvider.addItemAt(items[i], 0, 0);
            }
        }
    ...
    protected function proceedResult(items:Array, hasOldRecords:Boolean = false):void
    {
        if (_prependDataFunction != null)
        {
            _prependDataFunction(items);
        }
        else
        {
            ...
            for (var i:int = 0, n:int = items ? items.length : 0; i < n; i++)
            {
                _dataProvider.addItemAt(items[i], groupIndex, itemIndex + i);
            }
        }
...
//                }
    ...
    override protected function dataProvider_changeHandler(event:Event):void
    {
        ...
        if (_dataProvider.getLength() > 0 && _currentState == STATE_EMPTY)
        {
            setCurrentState(STATE_NORMAL);
        }
    }
